...
export const RoundProvider = ({ children, chainId, roundId }: { children: React.ReactNode; chainId: number; roundId: string }) => {
    const { data, isLoading, error } = useQuery({
        queryKey: ['round', chainId, roundId],
        queryFn: async () => {
            const response = await graphqlClient.request<{ rounds: Round[] }>(GET_ROUND, {
                chainId,
                id: roundId,
            });
    ...
    useEffect(() => {
        const processData = async () => {
            ...
            const token = chain?.tokens.find(token => token.address.toLowerCase() === data.matchTokenAddress.toLowerCase());
            ...
            const tokenPrice = await getTokenPrice(tokenSymbol, { chainId, address: data.matchTokenAddress as Hex});
            const matchAmount = formatUnits(BigInt(data.matchAmount), tokenDecimals || 18);
            const matchAmountInUsd = tokenPrice ? tokenPrice * Number(matchAmount) : data.matchAmountInUsd;
            const roundMetadata = typeof data.roundMetadata === 'string' 
            ...
            const plotData = {
                labels: applications.map(app => app.metadata.application.project.title),
