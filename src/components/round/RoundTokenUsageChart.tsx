...
export const RoundTokenUsageChart = () => {
  ...
  const { donationsData, tokenUsage } = roundData;

  if (!tokenUsage || Object.keys(tokenUsage).length === 0) {
    return wrapReturn(<div>No token usage data available for this round</div>);
  }
  ...
  return wrapReturn(
    <PieChart
      data={chartData}
      description="Distribution of tokens used for donations in this round"
